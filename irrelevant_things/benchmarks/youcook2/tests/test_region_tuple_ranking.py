...
from adaptive_search.schemas import RefinementHyperparameters, SparseCandidate, TemporalRegion
from benchmarks.youcook2.region_tuple_ranking import (
    RegionTupleParams,
...
class PoolEventRegionsTests(unittest.TestCase):
    def test_relative_delta_excludes_far_below_best(self) -> None:
        ...
        params = RegionTupleParams(relative_delta=0.15, max_regions_per_event=20)
    ...
    def test_absolute_threshold_excludes_even_if_within_delta(self) -> None:
        ...
        params = RegionTupleParams(absolute_threshold=0.18, relative_delta=0.5, max_regions_per_event=20)
        pooled = pool_event_regions(regions, event_id="e1", video_id="v1", params=params)
        self.assertEqual([r.id for r in pooled], ["r1"])
    ...
    def test_max_regions_per_event_caps_even_within_delta(self) -> None:
        ...
        params = RegionTupleParams(relative_delta=1.0, max_regions_per_event=3)
        pooled = pool_event_regions(regions, event_id="e1", video_id="v1", params=params)
...
class AssembleRegionTuplesTests(unittest.TestCase):
    ...
    def test_partial_coverage_zero_fills_missing_event_like_prioritize_videos(self) -> None:
        # e2 has no region at all for v1 - the video should still be scored
        # (matching prioritize_videos()'s zero-fill for an uncovered event),
        # not dropped entirely the way an earlier version of this code did.
        regions = [
        ...
        # An uncovered event can't violate order (no timestamp to compare).
    ...
    def test_order_weight_zero_score_equals_mean_of_best_regions(self) -> None:
        ...
        params = RegionTupleParams(relative_delta=1.0, order_weight=0.0)
        tuples = assemble_region_tuples_for_video("v1", ["e1", "e2"], regions, candidates_by_id, params)
        self.assertEqual(len(tuples), 2)  # 2 regions for e1 x 1 region for e2
        best = tuples[0]
    ...
    def test_order_weight_prefers_chronologically_consistent_tuple_on_tie(self) -> None:
        # Two same-mean-score tuples for e1/e2; only the in-order one (e1 before e2)
        # should win once order_weight > 0.
        regions = [
            _region("r1", event_id="e1", video_id="v1", candidate_ids=["c1"], normalized_coarse_score=0.8),
        ...
        params = RegionTupleParams(relative_delta=1.0, order_weight=0.2)
        tuples = assemble_region_tuples_for_video("v1", ["e1", "e2"], regions, candidates_by_id, params)
...
class OrderConstraintsTests(unittest.TestCase):
    ...
    def test_dropping_a_constraint_removes_it_from_scoring(self) -> None:
        # 2 pairs, second one inverted (3.0 -> 2.0). With both checked: (2*1-2)/2 = 0.
        self.assertEqual(_order_score([1.0, 3.0, 2.0], [(0, 1), (1, 2)]), 0.0)
        # Dropping the inverted pair leaves only the correct one: (2*1-1)/1 = 1.0.
    ...
    def test_constraint_direction_is_independent_of_list_position(self) -> None:
        # This is the scenario a reviewer flagged directly: if temporal_relation
        # says event 0 ("evt1") is *after* event 1 ("evt2") - i.e. the expected
        # constraint is predecessor=1 (evt2), successor=0 (evt1), t(evt2) < t(evt1)
        # - then timestamps ordered evt1 < evt4 < evt3 < evt2 (index order
        # [0,3,2,1] chronologically) VIOLATES that constraint (t(evt1)=1.0 is
        # NOT greater than t(evt2)=4.0), and must be penalized, not rewarded,
        # even though evt1 is listed before evt2 in the array.
        timestamps = [1.0, 4.0, 3.0, 2.0]  # evt1, evt2, evt3, evt4 by list position
        constraint_evt1_after_evt2 = [(1, 0)]  # predecessor=evt2(idx1), successor=evt1(idx0)
        self.assertEqual(_order_score(timestamps, constraint_evt1_after_evt2), -1.0)
        # A naive adjacent-list-position check (the earlier, buggy version of
        # this module) would instead have scored the (evt1,evt2) gap as
        # +1 (1.0 < 4.0, "increasing"), which is backwards relative to what
        # the relation actually claims.
        self.assertEqual(_order_score(timestamps, [(0, 1)]), 1.0)

    def test_dropped_constraint_removes_penalty_at_tuple_level(self) -> None:
        # Same fixture as the earlier "prefers chronologically consistent
        # tuple" test, but with no e1->e2 constraint at all (as if
        # temporal_relation said "independent") - both region choices for e2
        # should now score identically, since order no longer breaks the tie.
        regions = [
            _region("r1", event_id="e1", video_id="v1", candidate_ids=["c1"], normalized_coarse_score=0.8),
            _region("r2_early", event_id="e2", video_id="v1", candidate_ids=["c2e"], normalized_coarse_score=0.8),
            _region("r2_late", event_id="e2", video_id="v1", candidate_ids=["c2l"], normalized_coarse_score=0.8),
...
class ConfidenceGateTests(unittest.TestCase):
    def test_none_gate_is_unchanged(self) -> None:
        params = RegionTupleParams(order_weight=0.3, confidence_gate="none")
    ...
    def test_linear_gate_scales_with_confidence(self) -> None:
        params = RegionTupleParams(order_weight=0.3, confidence_gate="linear")
    ...
    def test_threshold_gate_cuts_off_below_threshold(self) -> None:
        params = RegionTupleParams(order_weight=0.3, confidence_gate="threshold", confidence_gate_threshold=0.5)
    ...
    def test_linear_gate_suppresses_order_bonus_for_weak_tuple(self) -> None:
        # Low region scores (weak signal) -> the order bonus should barely
        # move the score even with a large nominal order_weight.
        regions = [
        ...
        params_none = RegionTupleParams(relative_delta=1.0, order_weight=0.8, confidence_gate="none")
        params_linear = RegionTupleParams(relative_delta=1.0, order_weight=0.8, confidence_gate="linear")
        ...
        # order_score is -1.0 (e2's timestamp precedes e1's): ungated applies
        # the full order_weight=0.8 penalty (score = 0.1 - 0.8 = -0.7); the
        # linear gate scales that penalty down to order_weight*region_mean_score
        # = 0.8*0.1 = 0.08 (score = 0.1 - 0.08 = 0.02) - far less punishing.
...
class RankVideosByRegionTuplesTests(unittest.TestCase):
    def test_order_weight_zero_matches_prioritize_videos_ranking(self) -> None:
        # Two videos, two events; independent per-event best regions differ
        # in identity from any single "in order" combo to make the test
        # meaningful (video v2's best e1/e2 regions are NOT chronological).
        regions = [
        ...
        params = RegionTupleParams(relative_delta=1.0, order_weight=0.0, pooling="max")
